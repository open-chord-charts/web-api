...
(function(/*! Stitch !*/) {
  ...
  "bootstrapify/index": function(exports, require, module) {/* ===================================================
...
module.exports = jQuery;}, "json2ify/index": function(exports, require, module) {/*
...
}, "spine/lib/ajax": function(exports, require, module) {// Generated by CoffeeScript 1.3.3
(function() {
...
(function() {
...
}, "models/offline_local": function(exports, require, module) {(function() {
  var OfflineLocal;
  ...
  OfflineLocal = {
    ...
    saveLocal: function() {
      var cid, record;
      return window.localStorage[this.className] = JSON.stringify((function() {
        var _ref, _results;
        _ref = this.records;
        _results = [];
        for (cid in _ref) {
    ...
    loadLocal: function() {
      return this.refresh(window.localStorage[this.className] || [], {
        clear: true
      });
    }
  };
...
}, "models/chart": function(exports, require, module) {(function() {
  var Chart, OfflineLocal, Spine, transpose,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  OfflineLocal = require("models/offline_local").OfflineLocal;
  ...
  Chart = (function(_super) {
    ...
    Chart.configure("Chart", "composers", "genre", "key", "offline", "parts", "slug", "structure", "title");

    Chart.extend(OfflineLocal);

    Chart.extend(Spine.Model.Ajax.Methods);

    Chart.url = "/charts.json";

    Chart.fetchLocalOrAjax = function(params) {
      var refreshCallbacks;
      refreshCallbacks = Chart._callbacks.refresh;
      Chart.unbind("refresh");
      Chart.one("refresh", function(charts, options) {
        var chart, found;
        Chart._callbacks.refresh = refreshCallbacks;
        found = false;
        if (Chart.count() && params.query) {
          chart = Chart.findByAttribute(params.query.name, params.query.value);
          if (chart) {
            found = true;
          }
        }
        if (found) {
          return Chart.trigger("refresh", Chart.all(), {
            localStorage: true
          });
        } else {
          return Chart.fetchAjax(params);
        }
      });
      return Chart.fetch(params);
    };

    Chart.fetchAjax = function(params) {
      return this.ajax().fetch(params);
    };

    Chart.prototype.transpose = function(toKey) {
      var chord, chords, partName, _ref;
      if (toKey !== this.key) {
        _ref = this.parts;
        for (partName in _ref) {
          chords = _ref[partName];
          this.parts[partName] = (function() {
            var _i, _len, _results;
            _results = [];
            for (_i = 0, _len = chords.length; _i < _len; _i++) {
              chord = chords[_i];
              _results.push(transpose.transposeChord(chord, this.key, toKey));
            }
            return _results;
          }).call(this);
        }
...
}, "lib/transpose": function(exports, require, module) {(function() {
  var chordRegex, chromaticKeys, diatonicKeys, exports, getChromaticIndex, renderChord, transposeKey,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };
  ...
  chromaticKeys = [['G#', 'Ab'], ['A'], ['A#', 'Bb'], ['B'], ['C'], ['C#', 'Db'], ['D'], ['D#', 'Eb'], ['E'], ['F'], ['F#', 'Gb'], ['G']];

  diatonicKeys = ['A', 'B', 'C', 'D', 'E', 'F', 'G'];

  exports.decorateChart = function(chart) {
    var chord, chordIndex, newPart, partIndex, partName, parts, previousChord, _i, _j, _len, _len1, _ref, _ref1;
    parts = [];
    _ref = chart.structure;
    for (partIndex = _i = 0, _len = _ref.length; _i < _len; partIndex = ++_i) {
      partName = _ref[partIndex];
      newPart = {
        name: partName,
        chords: []
      };
      _ref1 = chart.parts[partName];
      for (chordIndex = _j = 0, _len1 = _ref1.length; _j < _len1; chordIndex = ++_j) {
        chord = _ref1[chordIndex];
        if (chordIndex > 0 && previousChord === chord || chart.structure.indexOf(partName) !== partIndex) {
          newPart.chords.push("—");
        } else {
  ...
  getChromaticIndex = function(key) {
    var keyIndex, keys, _i, _len;
    for (keyIndex = _i = 0, _len = chromaticKeys.length; _i < _len; keyIndex = ++_i) {
      keys = chromaticKeys[keyIndex];
  ...
  exports.partsToRows = function(parts) {
    var chord, index, newParts, part, rows, _i, _j, _len, _len1, _ref, _step;
    newParts = [];
    for (_i = 0, _len = parts.length; _i < _len; _i++) {
      part = parts[_i];
      rows = [];
      _ref = part.chords;
      for (index = _j = 0, _len1 = _ref.length, _step = 8; _j < _len1; index = _j += _step) {
        chord = _ref[index];
        rows.push(part.chords.slice(index, index + 8));
      }
      newParts.push({
        name: part.name,
        rows: rows
  ...
  exports.transposeChord = function(chord, fromKey, toKey) {
    var key, m, quality;
    if (fromKey === toKey) {
      return chord;
    }
  ...
  transposeKey = function(key, fromKey, toKey) {
    var chromaticOffsetDelta, diatonicOffsetDelta, keyChromaticOffset, keyDiatonicOffset, transposedKeyChromatic, transposedKeyDiatonic, transposedKeysChromatic, _i, _len;
    if (fromKey === toKey) {
      return key;
    }
    ...
    transposedKeyDiatonic = diatonicKeys[(keyDiatonicOffset + diatonicOffsetDelta + diatonicKeys.length) % diatonicKeys.length];
    transposedKeysChromatic = chromaticKeys[(keyChromaticOffset + chromaticOffsetDelta + chromaticKeys.length) % chromaticKeys.length];
    if (transposedKeysChromatic.length === 1) {
      return transposedKeysChromatic[0];
    } else {
      for (_i = 0, _len = transposedKeysChromatic.length; _i < _len; _i++) {
        transposedKeyChromatic = transposedKeysChromatic[_i];
...
}, "controllers/charts/show": function(exports, require, module) {(function() {
  var Chart, ChartsShow, OfflineButton, Spine, chartView, transpose,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  Chart = require("models/chart").Chart;

  chartView = require("views/chart");

  OfflineButton = require("controllers/offline_button").OfflineButton;

  transpose = require("lib/transpose");

  ChartsShow = (function(_super) {

    __extends(ChartsShow, _super);

    ChartsShow.prototype.elements = {
      ".actions": "actionsDiv",
    ...
    ChartsShow.prototype.logPrefix = "(Charts)";

    function ChartsShow() {
      this.render = __bind(this.render, this);

      this.onTransposeFormSubmit = __bind(this.onTransposeFormSubmit, this);

      this.onKeySelectChange = __bind(this.onKeySelectChange, this);

      this.onDeleteButtonClicked = __bind(this.onDeleteButtonClicked, this);

      this.onChartRefresh = __bind(this.onChartRefresh, this);
      ChartsShow.__super__.constructor.apply(this, arguments);
      this.routes({
        "/charts/:slug": function(params) {
          this.slug = params.slug;
          this.submitButton.detach();
          Chart.bind("change", this.render);
          Chart.bind("refresh", this.onChartRefresh);
          return Chart.fetchLocalOrAjax({
            query: {
              name: "slug",
              value: this.slug
            }
    ...
    ChartsShow.prototype.onChartRefresh = function(charts, options) {
      this.log("onChartRefresh", charts, options);
      this.chart = Chart.findByAttribute("slug", this.slug);
      if (this.chart) {
        if (this.offlineButton) {
          this.offlineButton.release();
        }
        this.offlineButton = new OfflineButton({
          chart: this.chart
        });
        this.actionsDiv.prepend(this.offlineButton.$el);
        return this.render();
      } else {
        return this.log("Chart \"" + this.slug + "\" not found");
      }
    };

    ChartsShow.prototype.onDeleteButtonClicked = function(event) {
      if (!confirm("Delete \"" + this.chart.title + "\"?")) {
        return event.preventDefault();
      }
    };

    ChartsShow.prototype.onKeySelectChange = function(event) {
      return this.transposeForm.trigger("submit");
    };

    ChartsShow.prototype.onTransposeFormSubmit = function(event) {
      event.preventDefault();
      return this.chart.transpose(this.keySelect.val()).save();
    };

    ChartsShow.prototype.render = function() {
      this.log("render");
      this.keySelect.val(this.chart.key);
      this.chordsDiv.html(chartView({
        partRows: transpose.partsToRows(transpose.decorateChart(this.chart.attributes()))
      }));
      return window.document.title = "" + this.chart.title + " (" + this.chart.key + ")";
    };
...
}, "controllers/offline_button": function(exports, require, module) {(function() {
  var Chart, OfflineButton, Spine,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  OfflineButton = (function(_super) {

    __extends(OfflineButton, _super);

    OfflineButton.prototype.attributes = {
      "data-toggle": "button",
      rel: "popover"
    };

    OfflineButton.prototype.className = "btn offline-button";

    OfflineButton.prototype.events = {
      "click": "onClick"
    };

    OfflineButton.prototype.logPrefix = "(OfflineButton)";

    OfflineButton.prototype.tag = "button";

    function OfflineButton(options) {
      this.render = __bind(this.render, this);

      this.onClick = __bind(this.onClick, this);
      OfflineButton.__super__.constructor.apply(this, arguments);
      this.el.text("Offline");
      if (this.chart.offline) {
    ...
    OfflineButton.prototype.onClick = function(event) {
      var newOfflineValue;
      newOfflineValue = !this.chart.offline;
      if (newOfflineValue) {
        this.log("Chart is now offline");
      } else {
        this.log("Chart is now online");
      }
      return this.chart.updateAttribute("offline", newOfflineValue);
    };

    OfflineButton.prototype.render = function() {
      var popover;
      popover = this.el.data("popover");
      if (this.chart.offline) {
...
}, "controllers/application_cache_info": function(exports, require, module) {(function() {
  var ApplicationCacheInfo, Spine,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  Spine = require("spine");

  ApplicationCacheInfo = (function(_super) {

    __extends(ApplicationCacheInfo, _super);

    ApplicationCacheInfo.prototype.logPrefix = "(ApplicationCache)";

    function ApplicationCacheInfo(options) {
      var _this = this;
      ApplicationCacheInfo.__super__.constructor.apply(this, arguments);
      window.applicationCache.oncached = function(event) {
        _this.log("cached", event);
        return _this.html("cached");
      };
      window.applicationCache.onchecking = function(event) {
        _this.log("checking", event);
        return _this.html("checking");
      };
      window.applicationCache.ondownloading = function(event) {
        _this.log("downloading", event);
        return _this.html("downloading");
      };
      window.applicationCache.onerror = function(event) {
        _this.log("error", event);
        return _this.html("error");
      };
      window.applicationCache.onnoupdate = function(event) {
        _this.log("noupdate", event);
        return _this.html("noupdate");
      };
      window.applicationCache.onobsolete = function(event) {
        _this.log("obsolete", event);
        return _this.html("obsolete");
      };
      window.applicationCache.onprogress = function(event) {
        var percentage;
        if (event.lengthComputable) {
          percentage = Math.round(event.loaded / event.total * 100) + '%';
          _this.log("progress " + percentage, event);
          return _this.html("progress " + percentage);
        } else {
          _this.log("progress", event);
          return _this.html("progress");
        }
      };
      window.applicationCache.onupdateready = function(event) {
        window.applicationCache.swapCache();
        _this.log("updateready", event);
        return _this.html("updateready");
      };
    }

    return ApplicationCacheInfo;
...
}, "controllers/navigator_info": function(exports, require, module) {(function() {
  var NavigatorInfo, Spine,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  NavigatorInfo = (function(_super) {

    __extends(NavigatorInfo, _super);

    NavigatorInfo.prototype.logPrefix = "(Navigator)";

    function NavigatorInfo(options) {
      this.onNavigatorOnline = __bind(this.onNavigatorOnline, this);

      this.onNavigatorOffline = __bind(this.onNavigatorOffline, this);
      NavigatorInfo.__super__.constructor.apply(this, arguments);
      window.ononline = this.onNavigatorOnline;
      window.onoffline = this.onNavigatorOffline;
      if (window.navigator.onLine) {
        this.onNavigatorOnline();
      } else {
        this.onNavigatorOffline();
      }
    }

    NavigatorInfo.prototype.onNavigatorOffline = function(event) {
      this.log("offline");
      return this.html("offline");
    };

    NavigatorInfo.prototype.onNavigatorOnline = function(event) {
      this.log("online");
      return this.html("online");
    };
  ...
  if (typeof module !== "undefined" && module !== null) {
    module.exports.NavigatorInfo = NavigatorInfo;
  }

}).call(this);
}, "views/chart": function(exports, require, module) {module.exports = function(__obj) {
  if (!__obj) __obj = {};
  ...
  (function() {
    (function() {
      var chord, part, row, rowIndex, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
    
      __out.push('<table class="table table-bordered table-striped">\n  <tbody>\n');
        ...
        for (rowIndex = _j = 0, _len1 = _ref1.length; _j < _len1; rowIndex = ++_j) {
          ...
          if (rowIndex === 0) {
            ...
            if (part.rows.length > 1) {
              __out.push('rowspan="');
              __out.push(__sanitize(part.rows.length));
              __out.push('"');
            }
            __out.push('>');
            __out.push(__sanitize(part.name));
            __out.push('</td>\n    ');
          }
          __out.push('\n    ');
  ...
  var $, App, ApplicationCacheInfo, ChartsShow, NavigatorInfo, Spine, start,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  ApplicationCacheInfo = require("controllers/application_cache_info").ApplicationCacheInfo;

  ChartsShow = require("controllers/charts/show").ChartsShow;

  NavigatorInfo = require("controllers/navigator_info").NavigatorInfo;
  ...
  App = (function(_super) {
    ...
    function App() {
      ...
      new ApplicationCacheInfo({
        el: this.el.find(this.applicationCacheInfoParagraph)
      });
      new NavigatorInfo({
        el: this.el.find(this.navigatorInfoParagraph)
      });
      new ChartsShow({
        el: this.el
      });

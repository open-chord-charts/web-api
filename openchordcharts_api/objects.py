...
class JsonMapper(object):
    ...
    def instance_to_json(self, state=None):
        return conv.object_to_clean_dict(self, state=state or conv.default_state)
    ...
    def to_json(self, state=None):
        return conv.check(conv.method('instance_to_json'))(self, state=state or conv.default_state)
...
class Wrapper(object):
    ...
    def get_collection(cls):
        ...
        if collection is None:
            assert cls.db is not None, u'Missing db in class %s' % cls
            collection_name = cls.get_collection_name()
            assert collection_name is not None, u'Missing mongo collection name in class %s' % cls
